        ...
        <div class="box-header">
           <h3 class="box-title">Data Barang Masuk</h3>
            <div class="pull-right">
                <a href="<?=site_url('stok/in/add')?>" class="btn btn-primary btn-flat">
                        ...
                        <tr>
                            ...
                            <td class="text-center" width="160px">
                                <a id ="set_dtl" class="btn btn-primary btn-xs" 
                                    ...
                                    data-suplayernama="<?=$data->suplayer_nama?>"
                                    data-jumlah="<?=$data->jumlah?>"
                                    data-tanggal="<?=indo_tanggal($data->tanggal)?>">
                                    <i class="fa fa-eye"></i> Detail
                                </a>
                                <a href="<?=site_url('stok/in/del/'.$data->stok_id.'/'.$data->item_id)?>" onclick="return confirm('Apakah anda yakin')" class="btn btn-danger btn-xs">
           ...
           <div class="modal-body table-responsive">
               <table class="table table-bordered no-margin">
                    <tbody>
                        ...
                        <tr>
                            <th>Pemasok</th>
                            <td><span id="suplayer_nama"></span></td>
                        </tr>
                        <tr>
                            <th>Jumlah</th>
...
$(document).ready(function() {
    $(document).on('click', '#set_dtl', function() {
        ...
        var suplayernama = $(this).data('suplayernama');
        var jumlah = $(this).data('jumlah');
        var tanggal = $(this).data('tanggal');
        $('#kodebarang').text(kodebarang);
        $('#item_nama').text(itemnama);
        $('#detail').text(detail);
        $('#suplayer_nama').text(suplayernama);
        $('#jumlah').text(jumlah);
